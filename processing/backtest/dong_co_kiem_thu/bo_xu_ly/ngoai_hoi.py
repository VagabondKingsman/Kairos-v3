"""Backtest engine for Forex (FX spot / CFD).

Market rules:
  - 24x5 trading (Sydney Monday open to New York Friday close)
  - Spread replaces explicit commission (bid-ask cost)
  - Leverage: configurable, typically 50:1 to 500:1
  - Standard lot = 100,000 base currency units
  - Overnight swap charged at end-of-day close
  - No price limits, no directional restrictions
  - PnL in quote currency (cross pairs converted via exit price)
"""

from __future__ import annotations
from datetime import datetime
from typing import Dict, Any

from processing.backtest.dong_co_kiem_thu.bo_xu_ly.co_so import BaseEngine


# ── Typical spread in pips (1 pip = 0.0001 for most pairs, 0.01 for JPY pairs) ──

_SPREAD_PIPS: dict[str, float] = {
    # Majors
    "EUR/USD": 1.0, "GBP/USD": 1.2, "USD/JPY": 1.0, "USD/CHF": 1.3,
    "AUD/USD": 1.2, "USD/CAD": 1.5, "NZD/USD": 1.5,
    # Crosses
    "EUR/GBP": 1.5, "EUR/JPY": 1.5, "GBP/JPY": 2.5, "EUR/CHF": 1.8,
    "AUD/JPY": 2.0, "CHF/JPY": 2.5, "EUR/AUD": 2.0, "GBP/AUD": 3.0,
    "EUR/CAD": 2.5, "GBP/CAD": 3.5, "AUD/CAD": 2.5, "NZD/JPY": 2.5,
    # Exotics (wider spread)
    "USD/TRY": 15.0, "USD/ZAR": 10.0, "USD/MXN": 8.0,
    "USD/SGD": 3.0, "USD/HKD": 3.0, "USD/CNH": 5.0,
}
_DEFAULT_SPREAD_PIPS = 2.0

# ── Daily swap points per standard lot (in account currency) ──
# Positive = receive, negative = pay. Values vary by broker/rate; simplified table.

_SWAP_LONG: dict[str, float] = {
    "EUR/USD": -6.5, "GBP/USD": -3.0, "USD/JPY": 8.0, "USD/CHF": 4.0,
    "AUD/USD": -2.0, "USD/CAD": 2.0, "NZD/USD": -1.5,
}
_SWAP_SHORT: dict[str, float] = {
    "EUR/USD": 3.5, "GBP/USD": -1.0, "USD/JPY": -12.0, "USD/CHF": -8.0,
    "AUD/USD": -1.0, "USD/CAD": -5.0, "NZD/USD": -2.0,
}

# Standard lot size
STANDARD_LOT = 100_000


def _pip_value(symbol: str) -> float:
    """Return the pip size for a currency pair.

    Args:
        symbol: Currency pair, e.g. 'EUR/USD', 'USD/JPY'.

    Returns:
        0.0001 for most pairs; 0.01 for JPY pairs.
    """
    quote = symbol.split("/")[1] if "/" in symbol else symbol[3:6]
    return 0.01 if quote.upper() == "JPY" else 0.0001


def _normalize_symbol(symbol: str) -> str:
    """Normalize a Forex symbol to 'XXX/YYY' format.

    Args:
        symbol: Raw symbol, e.g. 'EURUSD.FX', 'EUR/USD', 'EURUSD'.

    Returns:
        Normalized pair, e.g. 'EUR/USD'.
    """
    s = symbol.replace(".FX", "").replace(".", "").strip()
    if "/" in s:
        return s.upper()
    if len(s) == 6:
        return f"{s[:3]}/{s[3:]}".upper()
    return s.upper()


class ForexEngine(BaseEngine):
    """Forex engine for spot / CFD pairs.

    Config keys:
      - leverage: default 100.0 (100:1)
      - spread_pips_override: override spread for all pairs
      - lot_size: default 100000 (standard lot)
      - swap_enabled: default True
      - slippage_pips: additional slippage beyond spread, default 0.3
    """

    def __init__(self, config: dict):
        config = {**config, "leverage": config.get("leverage", 100.0)}
        super().__init__(config)
        self.spread_override = config.get("spread_pips_override")
        self.lot_size: float = config.get("lot_size", STANDARD_LOT)
        self.swap_enabled: bool = config.get("swap_enabled", True)
        self.slippage_pips: float = config.get("slippage_pips", 0.3)
        self._last_swap_dates: dict = {}  # per-symbol swap dedup

    def can_execute(self, symbol: str, direction: int, bar: Dict[str, Any]) -> bool:
        """Forex: 24x5 trading, no execution restrictions."""
        return True

    def round_size(self, raw_size: float, price: float) -> float:
        """Round to micro-lot (0.01 lot = 1,000 units).

        Position size is in currency units for PnL compatibility.
        """
        return max(int(raw_size / 1000) * 1000, 0)

    def calc_commission(self, size: float, price: float, direction: int, is_open: bool) -> float:
        """Forex: cost is embedded in spread via apply_slippage; no explicit commission.

        ECN brokers charge per-lot; defaulting to zero keeps the model simple.
        """
        return 0.0

    def apply_slippage(self, price: float, direction: int) -> float:
        """Apply half-spread + slippage using _active_symbol for the correct pip value."""
        return self.apply_slippage_for_symbol(self._active_symbol, price, direction)

    def apply_slippage_for_symbol(self, symbol: str, price: float, direction: int) -> float:
        """Apply slippage for a specific pair using exact spread.

        Args:
            symbol: Forex pair.
            price: Mid price.
            direction: 1 (buy) or -1 (sell).

        Returns:
            Execution price after slippage.
        """
        pair = _normalize_symbol(symbol)
        pip = _pip_value(pair)

        if self.spread_override is not None:
            spread_pips = self.spread_override
        else:
            spread_pips = _SPREAD_PIPS.get(pair, _DEFAULT_SPREAD_PIPS)

        total_pips = (spread_pips / 2) + self.slippage_pips
        return price + direction * total_pips * pip

    def on_bar(self, symbol: str, bar: Dict[str, Any], timestamp: datetime) -> None:
        """Apply daily swap at end-of-day settlement."""
        if not self.swap_enabled:
            return
        self._apply_swap(symbol, timestamp)

    def get_contract_multiplier(self, symbol: str) -> float:
        """Forex: multiplier is 1.0 (size already in currency units)."""
        return 1.0

    # ── Overnight Swap ──

    def _apply_swap(self, symbol: str, timestamp: datetime) -> None:
        """Credit/debit swap once per day at session close.

        Wednesday swap is tripled to cover the weekend (Saturday + Sunday).
        """
        # Support both native datetime and objects with a .date() attribute
        current_date = timestamp.date() if hasattr(timestamp, "date") else timestamp

        if self._last_swap_dates.get(symbol) == current_date:
            return
        self._last_swap_dates[symbol] = current_date

        pos = self.positions.get(symbol)
        if pos is None:
            return

        pair = _normalize_symbol(symbol)
        lots = pos.size / self.lot_size

        if pos.direction == 1:
            swap_per_lot = _SWAP_LONG.get(pair, -1.0)
        else:
            swap_per_lot = _SWAP_SHORT.get(pair, -1.0)

        # Wednesday (weekday==2): triple swap to cover Saturday + Sunday
        multiplier = 3.0 if (hasattr(timestamp, "weekday") and timestamp.weekday() == 2) else 1.0
        swap = lots * swap_per_lot * multiplier

        self.capital += swap