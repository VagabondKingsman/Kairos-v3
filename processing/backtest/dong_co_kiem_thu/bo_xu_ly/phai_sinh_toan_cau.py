"""Backtest engine for international futures (CME / ICE / Eurex).

Market rules:
  - Near 24/5 (CME Globex: Sunday 17:00 – Friday 16:00 CT, daily break 16:00-17:00)
  - Margin: initial + maintenance (exchange-set per contract)
  - Price limits: dynamic for equity indices, fixed for commodities
  - Contract multiplier: product-specific (ES=$50/pt, CL=$1000/bbl, GC=$100/oz)
  - Commission: per-contract (typically $1–3 per side)
  - Minimum size: 1 contract
  - Roll/expiry: not simulated (assumes continuous front-month data)
"""
...
# ── Contract multiplier (USD per point / unit) ──
...
# ── Initial margin per contract (USD estimate) ──
...
# ── Price limits (ratio vs. prior settlement) ──

_PRICE_LIMIT: dict[str, float] = {
    # Equity indices: Limit 1 at 7% (simplified to a single level)
    "ES": 0.07, "NQ": 0.07, "YM": 0.07, "RTY": 0.07,
    "MES": 0.07, "MNQ": 0.07,
    # Energy limits are dollar-based (~$10-15 for CL), not easily expressed as %, omitted
}

# ── Commission per contract (USD, per side) ──
...
def _extract_product(symbol: str) -> str:
    """Extract the product code from a futures contract symbol.
    ...
    Args:
        symbol: Futures contract symbol string.
    ...
    # Pattern 3: root product or fallback
...
class GlobalFuturesEngine(FuturesBaseEngine):
    ...
    Config keys:
      ...
      - commission_per_contract: override commission; default varies by product
    """

    def __init__(self, config: dict):
        # Most derivatives have 5–15% margin → 7–20x leverage.
        # Price is unknown at init time, so a reasonable fixed default is used.
        # Override via config["leverage"].
    ...
    def can_execute(self, symbol: str, direction: int, bar: Dict[str, Any]) -> bool:
        """T+0, long and short allowed; checks price limits for equity index contracts.
        ...
        if pct_chg is not None:
            ...
            if direction == -1 and pct_chg <= -limit + 0.001:
                return False  # Limit-down: cannot short
            if direction == 0:
    ...
    def round_size(self, raw_size: float, price: float) -> float:
        """Whole contracts only, minimum 1."""
        return max(int(raw_size), 0)

    def calc_commission(self, size: float, price: float, direction: int, is_open: bool) -> float:
        """Fixed per-contract fee (uses _active_symbol for product lookup)."""
    ...
    def calc_commission_for_symbol(
        ...
        Args:
            ...
            price: Execution price (unused — fee is flat per contract).
            is_open: Opening or closing the position.
    ...
    def apply_slippage(self, price: float, direction: int) -> float:
        """Slippage model for liquid international derivatives."""
...
def _calc_pct_change(bar: Dict[str, Any]):
    ...
    pct_chg field is only used as a last resort.
    """
    close = bar.get("close")
    ...
    if "pct_chg" in bar:
        val = bar["pct_chg"]
        # Pure Python NaN check (avoids pandas dependency)
        if val is not None and not (isinstance(val, float) and val != val):
            raw = float(val)
            # Heuristic: values > 1 are likely percentage points (e.g. 5.0 means 5%)
