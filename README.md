...
│   ├── live/                           # [NEW] Chạy tiền thật — Production Orchestrator
...
#### Môi trường Live Trading (`moi_truong_chay/live/`) — Production Orchestrator
...
* **Crash-Consistency (WAL First)**: Khi khôi phục sau crash, thứ tự bắt buộc là: Đọc WAL nội bộ (lấy lệnh in-flight) → Fetch sàn (lấy lệnh đã khớp) → So sánh. Nếu phát hiện orphan fill (sàn có position mà WAL không ghi nhận), symbol đó bị **HALT** ngay lập tức.

* **CoalescingQueue**: Queue đặc biệt cho reconciliation snapshots — khi đầy, tự động drop snapshot cũ nhất thay vì block producer. Đảm bảo hot-path không bao giờ bị chặn bởi reconciliation backlog.
