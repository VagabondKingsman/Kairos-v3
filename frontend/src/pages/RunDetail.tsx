...
export function RunDetail() {
  ...
  if (!run) return <div className="p-8 text-red-500">Không tìm thấy run</div>;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="border-b p-4 space-y-3">
        <div className="flex items-center gap-3">
          ...
          {run.elapsed_seconds && (
            <span className="text-xs text-muted-foreground">{run.elapsed_seconds.toFixed(1)}s</span>
          )}
        </div>
    ...
    <div className="p-4 space-y-4">
      {entries.map(([sym, bars]) => (
        <div key={sym}>
          ...
          <CandlestickChart
            data={bars}
            markers={run.trade_markers?.filter(m => m.code === sym)}
            indicators={run.indicator_series?.[sym]}
            height={500}
          />
        </div>
      ))}
...
function TradesTab({ run }: { run: RunData }) {
  ...
  if (trades.length === 0) {
    return <div className="p-8 text-muted-foreground text-sm">{t.noTrades}</div>;
  }
  return (
    <div className="p-4">
...
function CodeTab({ code }: { code: Record<string, string> }) {
  ...
  if (files.length === 0) {
    return <div className="p-8 text-muted-foreground text-sm">{t.noCode}</div>;
  }
  return (
    <div className="flex flex-col h-full">
