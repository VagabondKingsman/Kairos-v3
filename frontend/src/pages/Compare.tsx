...
const METRICS: MetricDef[] = [
  { key: "total_return",           label: "Tổng Lợi Nhuận",      type: "pct", higherIsBetter: true },
  { key: "annualized_return",      label: "LN Hằng Năm",         type: "pct", higherIsBetter: true },
  ...
  { key: "volatility",             label: "Biến Động",            type: "pct", higherIsBetter: false },
  ...
  { key: "avg_win",                label: "Thắng TB",             type: "pct", higherIsBetter: true },
  { key: "avg_loss",               label: "Thua TB",              type: "pct", higherIsBetter: false },
  { key: "trade_count",            label: "Số Lệnh",              type: "int", higherIsBetter: true },
  { key: "max_consecutive_losses", label: "Thua Liên Tiếp Max",   type: "int", higherIsBetter: false },
  { key: "exposure_time",          label: "Tỉ Lệ Nắm Giữ",       type: "pct", higherIsBetter: true },
  { key: "avg_holding_period",     label: "Giữ TB (ngày)",        type: "days", higherIsBetter: false },
];
...
function EquityChartOverlay({
  leftCurve, rightCurve, leftLabel, rightLabel,
}: {
  ...
  useEffect(() => {
    ...
    const PRIMARY_COLOR =
      getComputedStyle(document.documentElement).getPropertyValue("--chart-compare-a").trim() ||
      "#3b82f6";
    const SECONDARY_COLOR =
      getComputedStyle(document.documentElement).getPropertyValue("--chart-compare-b").trim() ||
      "#f5a623";
      ...
      <div className="flex gap-4 items-end">
        <div className="flex-1">
          ...
          >
            ...
            {runs.map((r) => (
              <option key={r.run_id} value={r.run_id}>{runLabel(r)} ({r.status})</option>
            ))}
          </select>
        </div>
                ...
                return (
                  <tr key={key} className="border-b last:border-0 hover:bg-muted/20">
                    ...
                    <td className={cn(
                      ...
                      diffClass(lv, rv, higherIsBetter)
                    )}>
                      {diffStr(lv, rv, type)}
                    </td>
                  </tr>
