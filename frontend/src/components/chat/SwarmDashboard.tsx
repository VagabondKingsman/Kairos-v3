...
function StatusLabel({ status }: { status: SwarmAgent["status"] }) {
  switch (status) {
    case "running": return <span className="text-primary font-medium">đang chạy</span>;
    case "done": return <span className="text-emerald-500 font-medium">xong</span>;
    case "failed": return <span className="text-red-500 font-medium">thất bại</span>;
    case "retry": return <span className="text-amber-500 font-medium">thử lại</span>;
    default: return <span className="text-muted-foreground/50">chờ</span>;
  }
}
    ...
    <div className="space-y-3 w-full">
          ...
          <div className="flex items-center gap-2">
            ...
            {finished ? (
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                ...
                {finalStatus === "completed" ? "HOÀN THÀNH" : "THẤT BẠI"}
              </span>
            ) : (
      ...
      {finalReport && (
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 px-5 py-4">
          <div className="text-xs font-semibold text-emerald-500 mb-3">Báo Cáo Tổng Hợp</div>
