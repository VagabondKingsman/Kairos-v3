...
export function EquityChart({ data, height = 300 }: Props) {
  ...
  useEffect(() => {
    ...
    chart.setOption({
      ...
      toolbox: {
        feature: {
          saveAsImage: { title: "Lưu" },
          restore: { title: "Reset" },
        },
        right: 8, top: 0,
        iconStyle: { borderColor: t.textColor },
      },
      legend: { data: ["Vốn", "Drawdown%"], textStyle: { color: t.textColor, fontSize: 11 }, right: 60, top: 4 },
      grid: [
  ...
  if (data.length === 0) {
    return <div className="text-muted-foreground text-sm p-4">Không có dữ liệu vốn</div>;
  }
