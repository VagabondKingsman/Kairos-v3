...
export function CandlestickChart({ data, markers, indicators, height = 500 }: Props) {
  ...
  useEffect(() => {
    ...
    const marks = (markers || []).map(m => ({
      ...
      value: m.side === "BUY" ? "M" : "B",
      name: [`${m.side === "BUY" ? "Mua" : "Bán"} @ ${m.price}`, m.qty ? `KL: ${m.qty}` : "", m.reason || ""].filter(Boolean).join("\n"),
    ...
    if (sub === "vol") {
      subSeries = [{ name: "KL", type: "bar", data: vol, xAxisIndex: 1, yAxisIndex: 1 }];
    ...
    chart.setOption({
        ...
        formatter: (params: any) => {
          ...
          for (const p of params) {
            if (p.seriesName === "K" && Array.isArray(p.value)) {
              ...
              html += `<br/>Mở: ${open.toFixed(2)}&nbsp; Cao: ${high.toFixed(2)}`;
              html += `<br/>Thấp: ${low.toFixed(2)}&nbsp; Đóng: <span style="color:${clr}"><b>${close.toFixed(2)}</b> ${chg >= 0 ? "+" : ""}${chg.toFixed(2)} (${chg >= 0 ? "+" : ""}${pct}%)</span>`;
            } else if (p.seriesName === "KL") {
              html += `<br/>KL: ${abbreviateNum(Number(p.value))}`;
      ...
      toolbox: {
        feature: { saveAsImage: { title: "Lưu" }, dataZoom: { title: { zoom: "Zoom", back: "Reset" } }, restore: { title: "Reset" } },
  ...
  if (data.length === 0) {
    return <div className="text-muted-foreground text-sm p-4">Không có dữ liệu giá</div>;
  }
    ...
    <div>
      <div className="flex items-center gap-2 mb-1 flex-wrap">
        ...
        <div className="relative">
          ...
          >
            Chỉ Báo ({overlays.size}) <ChevronDown className="h-3 w-3" />
          </button>
          {showMenu && (
