  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <h4 className="text-sm font-semibold">Kiểm Định Monte Carlo</h4>
        <Badge value={sig ? "Có Ý Nghĩa Thống Kê" : "Chưa Có Ý Nghĩa"} good={sig} />
      </div>
      <p className="text-xs text-muted-foreground">
        Xáo trộn thứ tự lệnh {mc.n_simulations.toLocaleString()} lần để kiểm tra Sharpe có tốt hơn ngẫu nhiên không.
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-xl border border-border/60 bg-muted/20 p-3">
        <Stat label="Sharpe Thực Tế" value={mc.actual_sharpe.toFixed(2)} />
        <Stat label="p-value (Sharpe)" value={mc.p_value_sharpe.toFixed(4)} sub={sig ? "< 0.05" : ">= 0.05"} />
        <Stat label="Sharpe Mô Phỏng TB" value={mc.simulated_sharpe_mean.toFixed(2)} sub={`std ${mc.simulated_sharpe_std.toFixed(2)}`} />
        <Stat label="Khoảng 90% Mô Phỏng" value={`[${mc.simulated_sharpe_p5.toFixed(2)}, ${mc.simulated_sharpe_p95.toFixed(2)}]`} />
      </div>
      <div className="space-y-1">
        <div className="flex justify-between text-[10px] text-muted-foreground font-mono">
          <span>P5: {mc.simulated_sharpe_p5.toFixed(2)}</span>
          <span>Thực Tế: {mc.actual_sharpe.toFixed(2)}</span>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Badge value={reliable ? "CI > 0" : "CI bao gồm 0"} good={reliable} />
      </div>
      <p className="text-xs text-muted-foreground">
        Tái lấy mẫu lợi nhuận hằng ngày {bs.n_bootstrap.toLocaleString()} lần để ước tính khoảng tin cậy {(bs.confidence * 100).toFixed(0)}%.
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-xl border border-border/60 bg-muted/20 p-3">
        <Stat label="Sharpe Quan Sát" value={bs.observed_sharpe.toFixed(2)} />
        <Stat label={`KTC ${(bs.confidence * 100).toFixed(0)}%`} value={`[${bs.ci_lower.toFixed(2)}, ${bs.ci_upper.toFixed(2)}]`} />
        <Stat label="Sharpe Trung Vị" value={bs.median_sharpe.toFixed(2)} />
...
function WalkForwardSection({ wf }: { wf: NonNullable<ValidationData["walk_forward"]> }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <h4 className="text-sm font-semibold">Phân Tích Walk-Forward</h4>
        <Badge value={`${wf.profitable_windows}/${wf.n_windows} giai đoạn lãi`} good={consistent ? true : wf.consistency_rate >= 0.5 ? null : false} />
      </div>
      <p className="text-xs text-muted-foreground">
        Chia thành {wf.n_windows} giai đoạn tuần tự để kiểm tra tính nhất quán.
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-xl border border-border/60 bg-muted/20 p-3">
        <Stat label="Nhất Quán" value={pctFmt(wf.consistency_rate)} />
        <Stat label="LN TB" value={pctFmt(wf.return_mean)} sub={`std ${pctFmt(wf.return_std)}`} />
        <Stat label="Sharpe TB" value={wf.sharpe_mean.toFixed(2)} sub={`std ${wf.sharpe_std.toFixed(2)}`} />
        <Stat label="Giai Đoạn" value={String(wf.n_windows)} />
      </div>
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b text-left text-muted-foreground">
            <th className="py-1.5 pr-3">#</th>
            <th className="py-1.5 pr-3">Giai Đoạn</th>
            <th className="py-1.5 pr-3 text-right">LN</th>
            <th className="py-1.5 pr-3 text-right">Sharpe</th>
            <th className="py-1.5 pr-3 text-right">Max DD</th>
            <th className="py-1.5 pr-3 text-right">Lệnh</th>
...
export function ValidationPanel({ data }: Props) {
  ...
  if (!hasMC && !hasBS && !hasWF) {
    return <p className="p-8 text-sm text-muted-foreground">Không có dữ liệu kiểm chứng.</p>;
  }
